...
describe('Validators', () => {
	...
	describe('url whitelist validator', () => {
		...
		it('accepts a whitelisted path and only the whitelisted path', () => {
			...
			const rules = {
				origins: [url_with_good_path.origin],
	...
	describe('login url validator', () => {
		it('prioritizes SUCCESSFUL_LOGIN_ORIGINS when ORIGIN_WHITELIST is also specified', () => {
			const legal_url = 'http://melk.no'
			const illegal_url = 'http://jest.no'

			const original_legal_urls = config.legal_urls
			config.legal_urls.login_redirect_origins = [legal_url]
			config.legal_urls.whitelist = [legal_url, illegal_url]

			expect(() => verifyLegalLoginUrl(legal_url)).not.toThrow()
			expect(() => verifyLegalLoginUrl(illegal_url)).toThrow()

			config.legal_urls = original_legal_urls
		})

		it('allows ORIGIN_WHITELIST when SUCCESSFUL_LOGIN_ORIGINS is not specified', () => {
			const original_legal_urls = config.legal_urls

			const legal_url = 'http://melk.no'
			const illegal_url = 'http://test.no'
			config.legal_urls.whitelist = [legal_url]
	...
	describe('Token validator', () => {
		it('accepts a correctly formated token', () => {
			const tokens = [
