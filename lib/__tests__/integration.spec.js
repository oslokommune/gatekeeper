...
async function initiateLoginFlow(gatekeeper_url, redirect_url) {
	 ...
	 */
	console.log(`Requesting login at ${gatekeeper_url} with redirect ${redirect_url}`)
	let headers
	try {
		const response = await axios.request({
			baseURL: 'http://localhost:4554',
			url: `/login?redirect=${encodeURIComponent(redirect_url)}`,
			method: 'GET',
			maxRedirects: 0
		})
...
describe('integration', () => {
	it('logs in and tests the received access_token against the userinfo endpoint', async () => {
		const redirect_url = 'http://localhost:3333'
