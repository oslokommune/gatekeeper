...
function verifyLegalNanoid(uuid) {
	if (!RE_NANOID_VALIDATOR.test(uuid))
		throw new ValidationError('Illegal Nanoid')
}
...
function verifyLegalCode(code) {
	if (!RE_CODE_VALIDATOR.test(code))
		throw new ValidationError('Illegal code characters')
}
...
function verifyLegalRedirectURL(rules, raw_url) {
	...
	if (url.origin !== rules.ORIGIN) {
		...
		error.targetOrigin = url.origin
		error.legalOrigin = rules.ORIGIN
	...
	if (!rules.PATHNAME_VALIDATOR.test(url.pathname)) {
		...
		error.targetPathname = url.pathname
		error.legalPathnameRegex = rules.PATHNAME_VALIDATOR
	...
	if (url.search !== '') {
		...
		error.targetQueryParams = url.search
		error.legalQueryParams = 'none'
