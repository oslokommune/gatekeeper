...
function setupTokenRefreshHandler(token_endpoint) {
	return async function middleware(req, res, next) {
		...
		if (hasExpired(current_refresh_token)) return next()
...
module.exports = (token_endpoint) => {
	const tokenRefreshHandler = setupTokenRefreshHandler(token_endpoint)

	return async (req, res, next) => {
		try {
			return await tokenRefreshHandler(req, res, next)
		}
		catch (error) {
			log.e(error)
