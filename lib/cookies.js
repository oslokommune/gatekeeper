...
const { ensureStrippedList } = require('./tools')
...
function setAccessToken(res, token, expires_in) {
	let expiry = new Date(Date.now())
	expiry.setSeconds(expiry.getSeconds() + expires_in)

	const domains = ensureStrippedList(process.env.TOKEN_COOKIES_DOMAINS)
	if (domains.length === 0) domains.push(BASE_URL.hostname)

	domains.forEach(domain => set(res, COOKIE_ACCESS_TOKEN, token, { domain, expires: expiry }))
}
...
function setRefreshToken(res, token) {
	...
	domains.forEach(domain => set(res, COOKIE_REFRESH_TOKEN, token, { domain }))
}
